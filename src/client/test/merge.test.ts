...
test('wraps both documents when the base document is empty', () => {
  expect(mergeMarkdown('', 'Server document', 'Device document')).toBe(
    '---\n\nServer document\n\n---\n\nDevice document\n\n---',
  )
})
...
test('keeps separate conflicts separated by an unchanged block', () => {
  expect(mergeMarkdown('A\n\nB\n\nC', 'Server A\n\nB\n\nServer C', 'Device A\n\nB\n\nDevice C')).toBe(
    ['---', 'Server A', '---', 'Device A', '---', 'B', '---', 'Server C', '---', 'Device C', '---'].join('\n\n'),
  )
})

test('preserves a server document that already contains a conflict', () => {
  const server = ['---', 'Original server', '---', 'First device', '---'].join('\n\n')

  const expected = ['---', '---', 'Original server', '---', 'First device', '---', '---', 'Second device', '---'].join(
    '\n\n',
  )
