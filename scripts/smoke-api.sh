...
curl --fail --silent \
  ...
  --data "{\"baseRevision\":null,\"note\":{\"id\":\"${note_id}\",\"title\":\"Smoke note\",\"content\":\"Hello **Markdown**\",\"tags\":[],\"resources\":[],\"createdAt\":1,\"updatedAt\":1,\"revision\":\"${revision_1}\"}}" \
...
curl --fail --silent \
  --request DELETE \
  --header 'Content-Type: application/json' \
  --data "{\"baseRevision\":\"${revision_1}\",\"revision\":\"${deleted_revision}\",\"updatedAt\":2}" \
...
curl --fail --silent \
  --request PUT \
  --header 'Content-Type: application/json' \
  --data "{\"baseRevision\":\"${deleted_revision}\",\"note\":{\"id\":\"${note_id}\",\"title\":\"Restored note\",\"content\":\"Restored\",\"tags\":[],\"resources\":[],\"createdAt\":1,\"updatedAt\":3,\"revision\":\"${revision_2}\"}}" \
...
curl --fail --silent "${origin}/api/sync?generation=${generation}&after=0" \
  | jq -e ".changes | map(select(.id == \"${note_id}\")) | length == 3" >/dev/null
