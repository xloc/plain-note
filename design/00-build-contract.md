...
- **R2 is authoritative storage.** Store each active note as `notes/<note-id>/note.md`, with human-readable Markdown content and essential metadata in front matter. Store attachments separately as `notes/<note-id>/resources/<resource-id>`; do not embed binary data in the note file. In `design/06-client-side-encryption.md`, E2E encryption is added. R2 stores the encrypted version.
